...
int16_t target_raw = 0;
int16_t target_temp = 0;
int16_t current_raw = 0;
int16_t current_raw_maxval = -32000;
int16_t current_raw_minval = 32000;
int16_t tt_maxval;
int16_t tt_minval;
int16_t target_bed_raw = 0;
int16_t current_bed_raw = 0;
unsigned long previous_millis_heater, previous_millis_bed_heater, previous_millis_monitor;
  ...
  int16_t temp_iState = 0;
  int16_t temp_dState = 0;
  int16_t prev_temp = 0;
  int16_t pTerm;
  int16_t iTerm;
  int16_t dTerm;
      //int output;
  int16_t error;
  int16_t heater_duty = 0;
  int16_t temp_iState_min = 256L * -PID_INTEGRAL_DRIVE_MAX / PID_IGAIN;
  int16_t temp_iState_max = 256L * PID_INTEGRAL_DRIVE_MAX / PID_IGAIN;
#endif
...
#ifdef WATCHPERIOD
  int16_t watch_raw = -1000;
  unsigned long watchmillis = 0;
#endif

#ifdef MINTEMP
  int16_t minttemp = temp2analogh(MINTEMP);
#endif

#ifdef MAXTEMP
  int16_t maxttemp = temp2analogh(MAXTEMP);
#endif
 ...
 void manage_heater()
 {
  ...
  //or it os better to deaktivate the uutput PIN or PWM ?
    ...
    if(current_raw >= maxttemp)
    {
      ...
      int16_t current_temp = analog2temp(current_raw);
      error = target_temp - current_temp;
      int16_t delta_temp = current_temp - prev_temp;
      
      prev_temp = current_temp;
      pTerm = ((long)PID_Kp * error) / 256;
      const int16_t H0 = min(HEATER_DUTY_FOR_SETPOINT(target_temp),HEATER_CURRENT);
      ...
      int16_t prev_error = abs(target_temp - prev_temp);
      int16_t log3 = 1; // discrete logarithm base 3, plus 1
