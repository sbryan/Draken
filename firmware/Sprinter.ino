...
#ifdef PIDTEMP
 uint16_t PID_Kp = PID_PGAIN, PID_Ki = PID_IGAIN, PID_Kd = PID_DGAIN;
#endif
...
volatile int16_t feedmultiply=100; //100->original / 200 -> Factor 2 / 50 -> Factor 0.5
int16_t saved_feedmultiply;
volatile bool feedmultiplychanged=false;
volatile int16_t extrudemultiply=100; //100->1 200->2
...
//unsigned ?? ToDo: Check
int16_t feedrate = 1500, next_feedrate, saved_feedrate;
  ...
  void fast_xfer()
  {
    ...
    *pstr = '\0';
    ...
    while(!done)
    {
      ...
      for(int16_t i=0;i<SD_FAST_XFER_CHUNK_SIZE+1;i++)
      {
        while(!Serial.available())
        {
        }
...
//------------------------------------------------
void analogWrite_check(uint8_t check_pin, int16_t val)
{
...
{ 
  ...
  while( filesize > sdpos && buflen < BUFSIZE)
  {
    ...
    read_char_int = (int16_t)serial_char;
...
{
  int16_t min_pin, max_pin, home_dir, max_length, home_bounce;
    ...
    switch( (int)code_value() ) 
    {
      ...
      case 42: //M42 -Change pin status via gcode
        if (code_seen('S'))
        {
          ...
          st_synchronize(); // wait for all movements to finish
#endif
          int16_t pin_status = code_value();
          if (code_seen('P') && pin_status >= 0 && pin_status <= 255)
          {
            int16_t pin_number = code_value();
            for(int16_t i = 0; i < sizeof(sensitive_pins) / sizeof(int); i++)
            {
    ...
    case 280: // M280 - set servo position absolute. P: servo index, S: angle or microseconds
      {
        ...
        if (code_seen('S')) {
          ...
          Serial.print(servo_index);
          showString(PSTR(": "));
          Serial.print(servos[servo_index].read());
          showString(PSTR(""));
        }
      }
      break;
...
int16_t calc_plannerpuffer_fill(void)
{
  int16_t moves_queued=(block_buffer_head-block_buffer_tail + BLOCK_BUFFER_SIZE) & (BLOCK_BUFFER_SIZE - 1);
  return(moves_queued);
}
...
static block_t *current_block = NULL;  // A pointer to the block currently being traced
...
void log_int(char* message, int16_t value) {
  Serial.print("DEBUG"); Serial.print(message); Serial.print(": "); Serial.println(value);
}
...
void log_uint(char* message, int16_t value) {
  Serial.print("DEBUG"); Serial.print(message); Serial.print(": "); Serial.println(value);
}
...
void log_int_array(char* message, int16_t value[], int16_t array_lenght) {
...
void log_long_array(char* message, long value[], int16_t array_lenght) {
  Serial.print("DEBUG"); Serial.print(message); Serial.print(": {");
  for(int16_t i=0; i < array_lenght; i++){
    Serial.print(value[i]);
    if(i != array_lenght-1) Serial.print(", ");
  }
  Serial.println("}");
...
void log_float_array(char* message, float value[], int16_t array_lenght) {
  Serial.print("DEBUG"); Serial.print(message); Serial.print(": {");
  for(int16_t i=0; i < array_lenght; i++){
    Serial.print(value[i]);
    if(i != array_lenght-1) Serial.print(", ");
...
void log_uint_array(char* message, uint16_t value[], int16_t array_lenght) {
  Serial.print("DEBUG"); Serial.print(message); Serial.print(": {");
  for(int i=0; i < array_lenght; i++){
    Serial.print(value[i]);
    if(i != array_lenght-1) Serial.print(", ");
...
void log_ulong_array(char* message, unsigned long value[], int16_t array_lenght) {
  Serial.print("DEBUG"); Serial.print(message); Serial.print(": {");
  for(int i=0; i < array_lenght; i++){
    Serial.print(value[i]);
    if(i != array_lenght-1) Serial.print(", ");
